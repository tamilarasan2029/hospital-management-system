...
Thank you for visiting Qwen Hospital!

For queries, contact: billing@qwenhospital.com
Phone: +91-9876543210
    `;
    ...
    element.setAttribute('download', `${bill.billId}.txt`);
    element.style.display = 'none';
    document.body.appendChild(element);
    element.click();
    document.body.removeChild(element);

    alert(`📥 Downloaded: ${bill.billId}.txt`);
  };
  ...
  return (
    <div>
      ...
      {/* ADD NEW BILL BUTTON - ONLY FOR ADMIN */}
      {user?.role === 'admin' && (
                  ...
                  <td style={{ padding: '15px' }}>
                    <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                      ...
                      {user?.role === 'admin' && bill.status === 'Pending' && (
                        <button
                          onClick={() => handleChangeStatus(bill.billId, 'Paid')}
                          style={{
                            padding: '6px 12px',
                            background: '#2ecc71',
                            color: 'white',
